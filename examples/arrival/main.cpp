...
struct Vehicle {
  ...
  SteeringOutput arrive(Vector2 target_position) {
    ...
    float clipped_speed = fminf(ramped_speed, max_speed);
    output.desired_velocity = Vector2Scale(offset, clipped_speed / distance);
...
int main() {
  ...
  constexpr float SLOWING_DISTANCE = 150;
  constexpr float STOP_RADIUS = 4;
  ...
  InitWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "Arrival");
  ...
  while (!WindowShouldClose()) {
    ...
    SteeringOutput steering = vehicle.arrive(mouse_position);
    ...
    DrawCircleLinesV(mouse_position, SLOWING_DISTANCE, DARKGRAY);
